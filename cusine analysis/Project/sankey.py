...
def sankey(left, right, leftWeight=None, rightWeight=None, colorDict=None,
    ...
    def get_alpha_values(c1,c2, threshold = 0.09):
        tot_cnf = (cnf_matrix.loc[c1,c2]+cnf_matrix.loc[c2,c1])
        if (cnf_matrix.loc[c1]+cnf_matrix.loc[:,c1]).nlargest(2)[-1] <= threshold and  \
            tot_cnf == (cnf_matrix.loc[c1]+cnf_matrix.loc[:,c1]).nlargest(2)[-1]:
            return 0.7
        if tot_cnf > threshold:
            return 0.7
        return 0.07
    ...
    for l in leftLabels:
        for l2 in rightLabels:
            ...
            if len(df[(df.left == l) & (df.right == l2)]) > 0:
                ...
                plt.fill_between(
                    np.linspace(0, xMax, len(ys_d)), ys_d, ys_u, alpha=get_alpha_values(l,l2), 
                    color=colorDict[lc]
                )
